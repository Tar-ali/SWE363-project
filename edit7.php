  ...
  <div class="body-dark" style="background-image: url('assets/img/gray.jpg');">
    <div class="container hero">
      <h1 style="padding:13px">Edit Portfolio &nbsp;</h1>
            ...
            require 'dbh.inc.php';
            $sql = 'SELECT  portTitle,p.portID  FROM (portfolio as p  join share as s on p.portID=s.portID) WHERE sharelink="' . $_GET['p'] . '"';
            $result = mysqli_query($conn, $sql);
            $row_users = mysqli_fetch_array($result);
            $GLOBALS['portID'] = $row_users['portID'];

            echo '<form method="POST" style="display:block;margin-bottom:50px;"><textarea name="newTitle" style="overflow-x:scroll;text-align:center;font-size:30px;margin:10px; width: 80%;height:8vh;;border-radius:10px; ">' . $row_users['portTitle'] . '</textarea> <button class="btn btn-primary" type="submit" value="subtitle" >Update</button><form>';
      ...
      <div class="btn-group align-content-center" role="group" style="text-align: center;">
        ...
        <script>
          ...
          function addSec(index) {
            ...
            // document.getElementById('cont').appendChild(helper);
            // document.getElementById('cont').appendChild(rmvBtn);
            document.getElementById('di').appendChild(di);
            ...
            index.parentNode.insert(di, index.parentNode.parentNode.nextSibling).scrollIntoView(); // add new element and focus on it.
